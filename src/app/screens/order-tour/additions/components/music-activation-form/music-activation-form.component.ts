...
export class MusicActivationFormComponent implements OnInit, OnDestroy {
  ...
  endHourEventSub: Subscription;
  startHourEventSub: Subscription;
  ...
  public onValueChange(event) {
    ...
    if (this.isItemOrderExist && this.editMode == true)
      this.form.disable({ emitEvent: false });
    this.supplierIdEventSub = this.form.controls["details"].get('supplierId').valueChanges.pipe(distinctUntilChanged()).subscribe(value => {
