...
export class MapsComponent implements OnInit {
  ...
  getColorGender(gender: string) {
    let res: any;
    if (gender == 'בנות')
      res = [210, 32, 186];
    else if (gender == 'בנים')
      res = [5, 114, 186];
    else if (gender == 'מעורב')
      res = [196, 196, 196];
    else
      res = [196, 196, 196];
    return res;
  ...
  layer_queryFeatures(response: any) {
    ...
    for (let i = 0; i < response.features.length; i++) {
      ...
      if (this.arrStruct.indexOf(buldingfeature.attributes.Purpose) != -1) {
        ...
        let svgPath: string = this.getSVG(buldingfeature.attributes.Purpose);
        ...
        if (objBin) {
          if (!this.viewGenderIcons) {
            stylePoint.outline = {
              color: this.getColorStatus(objBin.status),
              width: 1
            };
          }
          else {
            stylePoint.outline = {
              color: this.getColorGender(objBin.gender),
              width: 1
            };
          }
        }
